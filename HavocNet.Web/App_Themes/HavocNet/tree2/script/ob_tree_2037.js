...
function ob_t2_Add(parentId, childId, textOrHTML, expanded, image, subTreeURL)
{
	...
	if (document.getElementById(childId)) {
	    try{
	    //ob_t22(document.getElementById(childId));
	    }
	    catch(ex){}
	}
...
function ob_t2_Remove(childId, selectParent)
{
	...
	try
	{
		...
		if (nodeParent != null && selectParent == 1)
		{
			nodeParent.onclick();
		}
...
function ob_arrIdxOf(arr, obj) { for (var i = 0; i < arr.length; i++) {
        ...
        for(var i=0; i<arrTextBoxes.length; i++) 
        {
            ...
            {    
                ...
                if((iMainTop > iElTop && iMainTop < iElBottom) && (iMainLeft > iElLeft && iMainLeft < iElRight)) {
                    var ControlName = arrTextBoxes[i].id.replace('txt', '');
                    var ParentName = ob_getParentOfNode(document.getElementById(oDragableDiv.firstChild.firstChild.firstChild.childNodes[2].id)).innerText;
                    var ParentOfParentName = ob_getParentOfNode(ob_getParentOfNode(document.getElementById(oDragableDiv.firstChild.firstChild.firstChild.childNodes[2].id))).innerText;
                    
                    if (ControlName == ParentName.substring(0,ParentName.indexOf('<')) || ControlName == ParentOfParentName.substring(0,ParentOfParentName.indexOf('<')))
					{
                        arrTextBoxes[i].value = sMainText;
                    }
                    else
                    {
                        alert(sMainText + ' is not a valid member of the ' + ControlName + ' dimension');
                    }
                    return;
                }
            }
        ...
        for(var i=0; i<arrTextAreas.length; i++) 
        {
            ...
            if((iMainTop > iElTop && iMainTop < iElBottom) && (iMainLeft > iElLeft && iMainLeft < iElRight)) 
            {
				...
				try
				{
					if (copy == null || copy == 0)
					{
					    if(sMainText.substring(0,1) != '@')
					    {
					        arrTextAreas[i].value += sSpacer + sMainText;
					    }
					    else
					    {
					        arrTextAreas[i].value += sSpacer + document.getElementById('lblMessage3').innerText;
					    }
					}
					else
					{
